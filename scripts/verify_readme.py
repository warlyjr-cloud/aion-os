...
def main() -> None:
    ...
    cli_output = require_success([sys.executable, "-m", "src.cli", "start", "--once"], "CLI smoke run")
    if "service" not in cli_output or "aiond" not in cli_output:
        raise SystemExit(f"Unexpected CLI output:\n{cli_output}")

    health_output = require_success([sys.executable, str(ROOT / "scripts" / "health_check.py")], "Health check script")
    ...
    smoke_output = require_success([sys.executable, str(ROOT / "scripts" / "smoke_check.py")], "Smoke check script")
