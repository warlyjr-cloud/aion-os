...
1. Inteligência e autoridade permanecem separadas.
2. Segurança é guardrail, não componente da nota média.
3. Melhorias precisam acumular sem regressão de capacidades anteriores.
4. Resultados devem ser reconstruíveis por outra máquina e equipe.
5. Privacidade é local-first e saída de dados exige consentimento explícito.
6. Falha de modelo não pode desativar TCB, auditoria, stop ou rollback.
...
Sucesso não será declarado por uma demo. Exige gerações múltiplas, tarefas ocultas, baselines fortes, zero violações constitucionais observadas, reprodutibilidade e revisão externa.
