...
class GridManager:
    ...
    def broadcast_mutation(self, mutation_id: str, proof_payload: dict[str, Any]):
        ...
        for peer in peers:
            try:
                ...
                httpx.post(
                    f"{peer}/grid/gossip",
                    json={"mutation_id": mutation_id, "proof": proof_payload},
                    timeout=2.0,
                )
    ...
    def receive_gossip(self, payload: dict[str, Any]) -> dict[str, str]:
        ...
        # Se 10% de chance, simular que uma evolução concorrente já existia no mesmo 'timestamp relativo'  # noqa: E501
        if random.random() < 0.1:  # noqa: S311
            logging.warning(
                f"[\u2622\ufe0f MULTIVERSO] Colisão Relativística! Mutação {mutation_id} encontrou uma linha do tempo concorrente."  # noqa: E501
            )
            # Resolve the battle by fitness (simulated by random int for MVP)
            local_fitness = random.randint(1, 100)  # noqa: S311
            alien_fitness = random.randint(1, 100)  # noqa: S311
            if local_fitness > alien_fitness:
                logging.warning(
                    f"[\u2622\ufe0f MULTIVERSO] O Universo Local venceu a batalha (Fitness {local_fitness} > {alien_fitness}). Mutação {mutation_id} apagada da existência."  # noqa: E501
                )
                return {"status": "rejected", "reason": "timeline_battle_lost"}
            else:
                logging.warning(
                    f"[\u2622\ufe0f MULTIVERSO] O Universo Estrangeiro venceu (Fitness {alien_fitness} > {local_fitness}). A linha do tempo atual será sobreposta."  # noqa: E501
                )

        # Write to archive
        (gossip_dir / f"{mutation_id}.json").write_text(
    ...
    def offload_compute(self, objective: str, context: str) -> str | None:
        ...
        if random.random() < 0.3:  # noqa: S311
            peers = self.get_peers()
            if peers:
                peer = random.choice(peers)  # noqa: S311
                try:
