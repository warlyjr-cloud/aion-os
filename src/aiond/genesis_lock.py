...
def verify_dead_mans_switch(state_root: Path):
    ...
    """
    logging.info(
        "[\ud83d\udd12 G\u00caNESIS] Verifying Dead Man's Switch (Network Trust Anchor)..."
    )
    ...
    if not heartbeat_file.exists():
        ...
        logging.critical(
            "[\ud83d\udc80 FATAL] Genesis Heartbeat NOT FOUND! "
            "The network is physically isolated or compromised."
        )
        logging.critical(
            "[\ud83d\udc80 FATAL] Emulating network freeze... "
            "(In production, the daemon would abort here)."
        )
    ...
    if GENESIS_PUBLIC_KEY not in content:
        logging.critical(
            "[\ud83d\udc80 FATAL] Genesis Heartbeat signature invalid! Vampire attack detected."
        )
    ...
    logging.info(
        "[\ud83d\udd12 G\u00caNESIS] Heartbeat valid. "
        "Network operating under AION Labs jurisdiction."
    )
    return True
